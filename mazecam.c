...
int init_camera_device(int fd)
{
    ...
    {
        perror("Fail to ioctl stream_fmt");
        exit(EXIT_FAILURE);
    }
...
//将采集好的数据放到文件中
int process_image(void *addr, int length, int index)
{
    FILE *fp;
    ...
    sprintf(picture_name, "pictures/zyeeda-%llu.jpg", time_in_mill);
...
int read_frame(int fd, int index)
{
    struct v4l2_buffer buf;
    unsigned int i;
    ...
    //读取进程空间的数据到一个文件中
    process_image(user_buf[buf.index].start,user_buf[buf.index].length, index);
